...
[all:vars]
ansible_user=master #우분투는 아마존 들어 갈 때 쓰던 거
ansible_become=yes # sudo 권한 사용
```

-----
### 012.k8s설치하기.md와 비교해서 설명: apt에 직접 쳐서 했던 것 들이 yaml에서는 어떻게 처리하는지 확인
...
<img width="1920" height="1040" alt="image" src="https://github.com/user-attachments/assets/1006eb3e-0f85-4f0a-8910-69788d8e480e" />
<img width="1920" height="1040" alt="image" src="https://github.com/user-attachments/assets/712548b7-01ef-4be3-9a16-eae2b640bf10" />
<img width="1920" height="1040" alt="image" src="https://github.com/user-attachments/assets/2936120c-ece7-4587-8d46-d4e6f3ff9826" />
...
- name: ⚙️ Kubernetes Pre-requisites Setup
  ...
  tasks:
    ...
    - name: Install kubelet, kubeadm, kubectl and hold their versions
      ansible.builtin.apt:
        name: 
          - kubelet={{ kube_version }} #jinja2변수명 
          - kubeadm={{ kube_version }}
          - kubectl={{ kube_version }}
        #깔려있으면 놔두기
        state: present
        #버전은 유지하고 업데이트만 : 버전을 마지막 끝자리까지 하는지, 중간까지만 하는지도 다름:프로그래머의 고려사항에 있을 수 있음
        #버전의 마지막 끝자리는 윗 버전에 영향을 주지않는다는 뜻
        #버그 수정 해보는 포인트!
        update_cache: yes

    - name: Mark K8s packages to hold (prevent accidental upgrade)
      #dpkg는 옛날에 쓴 말로 패키지를 말함
      ...
      loop:
        ...
        - kubectl
```
+ 버전 관리: 3.10.11일 때, 완전히 main이 바꼈을 때는 가장 첫 번째 자리 숫자가 바뀜 : ex) 4.0.0
  ...
  tasks:
    - name: Initialize the Kubernetes Cluster (kubeadm init)
      ...
      ansible.builtin.command: >
        kubeadm init 
        --pod-network-cidr={{ pod_network_cidr }}  # 여기서 선언: 위의 Calico CNI 표준 CIDR에서 변수 가져옴
    ...
    # Calico CNI 설치 (네트워크 플러그인) # 다르면 처리 할 수 있을지 봅시다~
    # 네트워크 카드는 프로토콜로 움직임 다르면 통신이 안될까? 같은 쿠버네티스면 같은 걸로 가는 게 나음 특히 로드발란싱 할 때
    # 하지만 지금은 마스터와 노드니까 어떨지 보자
    # 노드에서 더 좋은 것을 썼음 마스터랑 다른데 어떻게 될 지 봅시다~
