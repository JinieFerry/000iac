...
+ 마스터 모드에서 들어갈 수 있게 키 복사해두기  

+ 마스터 패스워드와 root 패스워드 바꾸기 :jj12345678
```
#위치 vm1:~/.ssh
#root 패스워드 바꾸기 :jj12345678
$ sudo passwd root

#마스터 패스워드 바꾸기 :putto4us
$ passwd
```
...
아래에서 위의 코드로 수정됨 : Cisco CLI -> Flannel CLI로 변경
...
# 확인해봅쉬다
#마스터노드에서 토큰 생성하기
$ kubeadm token create --print-join-command #치면 키 값이 나올 것
# 결과값이 나오면 메보장을 열어서 복사해 둡니다.
  ...
  tasks:
    ...
    - name: Execute join command on worker nodes
      ansible.builtin.shell:
        #cmd: "{{ hostvars['k8s-master']['join_command_result']['stdout'] }}" #원래코드
    
        cmd: "" #따옴표 안에 위에서 메모장에 복사해 둔 키를 넣음
