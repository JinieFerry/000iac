...
https://github.com/putto4u/04.PrivateCloudInfra/blob/main/08.%EB%8F%84%EC%BB%A4%20%EC%BB%B4%ED%8F%AC%EC%A6%88/005.%EB%8F%84%EC%BB%A4%EC%BB%B4%ED%8F%AC%EC%A6%88%20%EC%9D%B4%ED%95%B4%EC%99%80%20%EC%8B%A4%EC%8A%B5.md## Docker Compose 개요 및 구조

**Docker Compose**는 여러 개의 컨테이너로 구성된 복잡한 애플리케이션을 정의하고 실행하기 위한 도구입니다. YAML 파일을 사용하여 애플리케이션의 서비스, 네트워크(파일과 다른 점), 볼륨을 설정하며, 단 한 번의 명령어로 전체 스택을 제어할 수 있습니다.
...
가장 기본적인 구조입니다. 웹 서버가 백엔드 API 서버(=백엔드 서버,DB서버와 연결되어 있다)와 통신합니다.

```yaml
version: '3.8'
#서비스가 두 개 (프론트/백엔드:이미지 2개)<-통신하게 아이피도 부여 가능(네트워크 드라이버로 브릿지 사용)
services:
  frontend:
    image: nginx:alpine #태그라고 부름(버전말고 apline같은 것도 씀)
    #포트포워딩을 위해 포트번호 써 줌 <> 도커파일에서는 EXPOSE
    ports:
      - "80:80"    
    networks:
...
services:
  frontend:
    ...
    ports:
      - "80:80"
    # 볼륨 추가 됨 (도커 파일에서도 볼륨), 수정하지 말라고 추가해둠
    volumes:
  ...
  backend:
    ...
    # 나중에 vault로 변수값 추가 함(지금은 이해하기 쉽게)
    environment:
