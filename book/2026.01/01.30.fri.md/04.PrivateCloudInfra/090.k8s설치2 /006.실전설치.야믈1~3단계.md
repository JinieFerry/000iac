...
```
ssh되는 디렉토리끼리 파일 통째로 보내기 :vmmastere1에서 실행
```
...
vm1 플레이북 실행 트러블슈팅
```
#지금 ansible-playbook을 실행하는 머신:(vm1)에서 sshpass 설치
sudo apt-get update
...
```
#vmmaster1에서 발행
kubeadm join 192.168.115.251:6443 --token abcdef.0123456789abcdef \
	--discovery-token-ca-cert-hash sha256:db2f7e195d44ab844fd3bf7651996c4c12c630bd568403270e0de4e119d73736 

#vm1에서 실행
sudo kubeadm join 192.168.115.251:6443 --token 6aiib6.ncg8mey9fx9mbq0g --discovery-token-ca-cert-hash sha256:db2f7e195d44ab844fd3bf7651996c4c12c630bd568403270e0de4e119d73736 
...
### 마스터 확인: 04.PrivateCloudInfra/10. k8s 구성과 운영
/000.k8s로 Hello word.md
```
