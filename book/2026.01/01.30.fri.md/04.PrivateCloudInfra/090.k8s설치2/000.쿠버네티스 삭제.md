...
sudo rm -rf /etc/cni/
sudo rm -rf /opt/cni/bin #실행파일 라이브러리
...
# cni는 컨테이너를 위한 가상 인터페이스라 다 link로 되어있음
# 리눅스는 다 파일로 처리하기 때문에 물리적 네트워크 카드도 다 파일로 처리 함
sudo ip link delete cni0
sudo ip link delete flannel.1
# 길어서 cali...로 줄여서 쓴 것(26.01.30.금 기준으로 아직 실습에서 칼리코 안 깔았음)
# 지피티로 찾으면 전체 명칭 볼 수 있음
sudo ip link delete cali... # Calico 사용 시

# 아이피 테이블이 리눅스에서 가장 중요한 정책으로 파이어월이라고 봐도 됨
# fw는 아이피 테이블 치는 게 너무 어려움(차단,드랍,허용 등등)
# fw을 쉽게 만든 게 ufw:실제 처리는 ip tables가 하고 ufw는 넘기는 일을 함
# overlay로 다 1을 줘서 가상의 네트워크 카드를 모두 true로 줬던 것을 ip tables가 처리함
# IP 테이블(iptables) 초기화
sudo iptables -F && sudo iptables -t nat -F && sudo iptables -t mangle -F && sudo iptables -X

```

+ 시스템 엔지니어는 건물 근간을 짓기 때문에 근간부터 수정해야 함     
+ 소프트웨어적으로 아직 프로토콜 규격 standard를 안 만들어서 칼리코는 acl만들 듯이 정책을 만들 수 있게 더 복잡하게 만들엇 기능이 많음,플라넬은 간단하게 만들었음
