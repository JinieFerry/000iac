##### 성의진 실습 에러
<img width="1912" height="1041" alt="image" src="https://github.com/user-attachments/assets/9f1f487f-5061-401d-b24f-68751d0a2124" />
배포는 성공했는데 
<img width="625" height="210" alt="image" src="https://github.com/user-attachments/assets/57cb97d5-571d-4f65-865d-4d3d172f031c" />
이전 텍스트가 사이트에 뿌려짐  

-> nginx 이미 깔려있으면 깔지 마라고 해서 이전버전 그대로 일 수 있음
...
```bash
# 1. 작업 디렉토리 생성 및 이동
mkdir -p ~/projects/web_dep #자기꺼 넣어야 함 (1516:KJJ-art랑jinieferry 협업실습하는 수업용 리퍼지토리)
cd ~/projects/web_dep #자기꺼 넣어야 함 (1516)
...
-----------------
### 재정님 버전 inventory.ini만드는 단계부터
🔁 재정님 서버 기준 (역방향 실습용)
1️⃣ inventory.ini (재정님 서버에 위치)
```
[webservers]
web-node-1 ansible_host=192.168.10.187
...
2️⃣ deploy.yml (교재 동일, URL만 유지)

GitHub 레포는 JinieFerry/1516 그대로
(재정님은 collaborator → pull 권한 있음)

```
---
- name: 1516 자동 배포 플레이북
  hosts: webservers
  become: true
  vars:
...
4️⃣ 너(대상 서버)에서 확인
`curl http://192.168.10.187`

요약 (한 줄)

소스 주체: 재정님 서버

코드 출처: GitHub JinieFerry/1516

대상 서버: 너 192.168.10.187
