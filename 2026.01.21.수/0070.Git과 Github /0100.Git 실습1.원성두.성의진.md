...
### 이제 부터 실습 자기 깃에서 하기 아예 자기 계정에서 하기 
### comflict는 다른 사람이랑 같은 파일로 할 것
...
---
### iac에서 하지 말고 별도의 디렉토리 만들어서 혼자 실습하고 싹다 지우기 -> temp라고 하는 경우도 있는데 임시로 할 때는 누가 봐도 지워도 된다는 의미로 imsi라고 함
### 하지만 홈에서는 절대 만들지 말기 (홈에서 만들어버리면 전체를 관리하겠다는 뜻이어서 로그가 엄청 길게 나옴)
### 중요 -^0 실습해보기 이것만 하고 지워도 될 것 같습니다^^
...
git status #상태가 어떻게 찍히는지 보기
git add README.md #git add로도 할 수 있지만 파일이 여러개 일 경우, git add . 만 해도 됨 (스테이징 하는 것)
git commit -m "chore: initial commit"
git log --oneline #-^0 중요 항상 깃 로그를 읽어야 함 (git commit 전에 로그 한 번 보고, 수정 후에도 로그 한 번 보고)
```
...
git status # 추가 된 것 과 빠진 것을 어떻게 표시하는지 꼭 보기 중요 #-^0
...
- `git diff --staged`는 “스테이징 된 변경” 커밋 해버리면 staged가 없음 (버전 관리가 된 것)


C. 커밋 vs 커밋 (History Comparison)
과거의 특정 시점과 현재, 혹은 특정 버전 간의 차이를 비교할 때 사용한다.

명령어:

```Bash

# 최신 커밋(HEAD)과 바로 이전 커밋(HEAD^) 비교
git diff HEAD^ HEAD
...
목표: “망쳤을 때 복구” 2가지 루트 익히기 프로그램 같은 경우에는 커밋 됐는지 아닌지 차이를 알아야 함
...
git branch -M main # 우리는 깃 브랜치 master로 했는데 깃허브는 main으로 되어있어서 서로 같은거로 안봐서 push 할 때 매칭이 안됨 -> 내 것을 main으로 바꿔주는 것
...
```bash
# 1. README.md 파일 끝에 "wip line"이라는 텍스트를 추가합니다.
# (WIP: Work In Progress, 아직 작업 중이라는 뜻)
echo "wip line" >> README.md

# 2. 현재 작업 디렉토리의 상태를 확인합니다. 
# README.md 파일이 수정되었다(modified)는 메시지가 출력됩니다.
git status

# 3. 현재의 변경 사항을 임시 저장소(Stash Stack)에 push하여 보관합니다.
# -m 옵션을 통해 나중에 찾기 쉽도록 "WIP: readme edit"이라는 메모를 남깁니다.
# 이 명령을 실행하면 작업하던 내용은 사라지고 마지막 커밋 상태로 깨끗해집니다.
git stash push -m "WIP: readme edit"

# 4. 다시 상태를 확인합니다. 
# 수정 중이던 내용이 스태시에 보관되었으므로 "working tree clean" 상태가 됩니다.
git status

# 5. 현재 스태시 저장소에 보관된 작업 목록을 보여줍니다.
# stash@{0}: On main: WIP: readme edit 와 같은 형태로 출력됩니다.
git stash list

# 6. 스태시 목록에서 가장 최근에 저장한 작업(0번)을 꺼내어 현재 브랜치에 적용합니다.
# pop은 적용과 동시에 스태시 목록에서 해당 기록을 삭제합니다.
git stash pop

# 7. 최종 상태를 확인합니다.
# 꺼내온 작업 내용이 다시 복구되어 README.md가 수정된 상태로 돌아옵니다.
git status
```
